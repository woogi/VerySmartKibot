...
public class CamDatabase {
	...
	Log.d(TAG,
		"-------------------getClassIDMatches-------------------------");
	String[] columns = new String[] { BaseColumns._ID, KEY_CLASS_ID,
		KEY_TAG_NAME, KEY_FRIEND_LEVEL, KEY_TRAINING_IMAGE_PATH };
	...
	Log.d(TAG, "-------------------query success------cursor.getCount()="
		+ cursor.getCount());
	db.close();
	return cursor;
    }

    public void addItem(int classID, String tagName, int countInClass,
	    String dirPath, String imageFilepath, int friendLevel) {
	long addedID;
	...
	addedID = mDatabaseOpenHelper.getWritableDatabase().insert(
		DATABASE_TABLE_NAME, null, initialValues);
	...
	mDatabaseOpenHelper.getWritableDatabase().delete(DATABASE_TABLE_NAME,
		KEY_CLASS_ID + "=" + classID, null);
	...
	private static final String FTS_TABLE_CREATE = "create table "
		+ DATABASE_TABLE_NAME + " ("
		+ "_id integer primary key autoincrement, " + KEY_CLASS_ID
		+ " int not null, " + KEY_TAG_NAME + " text not null,"
		+ KEY_COUNT_IN_CLASS + " int not null," + KEY_DIRECTORY_PATH
		+ " text not null," + KEY_TRAINING_IMAGE_PATH
